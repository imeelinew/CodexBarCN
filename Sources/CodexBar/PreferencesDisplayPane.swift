...
struct DisplayPane: View {
        ...
        ScrollView(.vertical, showsIndicators: true) {
            VStack(alignment: .leading, spacing: 16) {
                SettingsSection(contentSpacing: 12) {
                    Text(PrototypeChineseLocalization.text("Menu bar"))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .textCase(.uppercase)
                    PreferenceToggleRow(
                        title: PrototypeChineseLocalization.text("Merge Icons"),
                    ...
                    PreferenceToggleRow(
                        title: PrototypeChineseLocalization.text("Switcher shows icons"),
                        subtitle: PrototypeChineseLocalization.text(
                    ...
                    PreferenceToggleRow(
                        title: PrototypeChineseLocalization.text("Show most-used provider"),
                        subtitle: PrototypeChineseLocalization.text(
                        ...
                        .opacity(self.settings.mergeIcons ? 1 : 0.5)
                    PreferenceToggleRow(
                        title: PrototypeChineseLocalization.text("Menu bar shows percent"),
                        subtitle: PrototypeChineseLocalization.text(
                            "Replace critter bars with provider branding icons and a percentage."),
                        binding: self.$settings.menuBarShowsBrandIconWithPercent)
                    HStack(alignment: .top, spacing: 12) {
                        VStack(alignment: .leading, spacing: 4) {
                            Text(PrototypeChineseLocalization.text("Display mode"))
                                .font(.body)
                        ...
                        Spacer()
                        Picker(
                            PrototypeChineseLocalization.text("Display mode"),
                ...
                SettingsSection(contentSpacing: 12) {
                    Text(PrototypeChineseLocalization.text("Menu content"))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .textCase(.uppercase)
                    PreferenceToggleRow(
                        title: PrototypeChineseLocalization.text("Show usage as used"),
                        subtitle: PrototypeChineseLocalization.text(
                            "Progress bars fill as you consume quota (instead of showing remaining)."),
                        binding: self.$settings.usageBarsShowUsed)
                    PreferenceToggleRow(
                        title: PrototypeChineseLocalization.text("Show reset time as clock"),
                        subtitle: PrototypeChineseLocalization.text(
                            "Display reset times as absolute clock values instead of countdowns."),
                        binding: self.$settings.resetTimesShowAbsolute)
                    PreferenceToggleRow(
                        title: PrototypeChineseLocalization.text("Show credits + extra usage"),
                        subtitle: PrototypeChineseLocalization.text(
                            "Show Codex Credits and Claude Extra usage sections in the menu."),
                        binding: self.$settings.showOptionalCreditsAndExtraUsage)
                    PreferenceToggleRow(
                        title: PrototypeChineseLocalization.text("Show all token accounts"),
                        subtitle: PrototypeChineseLocalization.text(
    ...
    private var overviewProviderSelector: some View {
        VStack(alignment: .leading, spacing: 6) {
            HStack(alignment: .center, spacing: 12) {
                Text(PrototypeChineseLocalization.text("Overview tab providers"))
                    .font(.body)
                Spacer(minLength: 0)
                if self.showsOverviewConfigureButton {
                    Button(PrototypeChineseLocalization.text("Configure…")) {
            ...
            if !self.settings.mergeIcons {
                Text(PrototypeChineseLocalization.text("Enable Merge Icons to configure Overview tab providers."))
                    .font(.footnote)
                    .foregroundStyle(.tertiary)
            } else if self.activeProvidersInOrder.isEmpty {
                Text(PrototypeChineseLocalization.text("No enabled providers available for Overview."))
                    .font(.footnote)
                    .foregroundStyle(.tertiary)
            } else {
    ...
    private var overviewProviderPopover: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text(PrototypeChineseLocalization.chooseUpToProviders(Self.maxOverviewProviders))
                .font(.headline)
            Text(PrototypeChineseLocalization.overviewOrderHint())
                .font(.footnote)
                .foregroundStyle(.tertiary)
    ...
    private var overviewProviderSelectionSummary: String {
        ...
        guard !selectedNames.isEmpty else { return PrototypeChineseLocalization.text("No providers selected") }
