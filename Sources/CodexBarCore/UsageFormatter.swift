...
public enum UsageFormatter {
    public static func usageLine(remaining: Double, used: Double, showUsed: Bool) -> String {
        ...
        return PrototypeChineseLocalization.percentLabel(clamped, showUsed: showUsed)
    }
    ...
    public static func resetDescription(from date: Date, now: Date = .init()) -> String {
        ...
        return date.formatted(.dateTime.year().month().day().hour().minute().locale(Locale(identifier: "zh_Hans")))
    }
        ...
        now: Date = .init()) -> String?
    {
        ...
        if let desc = window.resetDescription {
            ...
            if trimmed.lowercased().hasPrefix("resets") { return PrototypeChineseLocalization.text(trimmed) }
            return "\(trimmed)重置"
        }
        return nil
    }
    ...
    public static func updatedString(from date: Date, now: Date = .init()) -> String {
        ...
        if let hours = Calendar.current.dateComponents([.hour], from: date, to: now).hour, hours < 24 {
            ...
            return "\(rel.localizedString(for: date, relativeTo: now))更新"
            #else
